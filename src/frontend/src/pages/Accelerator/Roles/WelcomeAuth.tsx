...
const WelcomeAuth: React.FC = () => {
  ...
  // Invite details passed as URL params
  const acceleratorName = searchParams.get('accelerator') || 'Your Accelerator';
  ...
  useEffect(() => {
    // optional: check if already authenticated
  }, []);
  ...
  const handleAuth = async (method: 'ii' | 'nfid') => {
    ...
    try {
      ...
      // Link identity with startup account
      if (startupEmail && founderName) {
        ...
        if (linkResult !== true) {
          ...
          toast.warning('Authenticated, but failed to link account. Try again later.');
        } else {
      ...
      <div className="bg-white rounded-lg shadow-md max-w-md w-full p-6 text-center">
        <div className="mb-6">
          ...
          <p className="text-gray-600 text-sm mb-2">
            You’ve successfully joined <strong>{acceleratorName}</strong> as <strong>{startupName}</strong>.
          </p>
...
import { loginWithII, loginWithNFID } from '../../../services/auth';
import { linkStartupPrincipal } from '../../../services/startup-invite';
import { getTeamInviteByToken } from '../../../services/team'; // adjust import path
import { toast } from 'react-toastify';
